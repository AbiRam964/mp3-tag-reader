...
int mp3_view(char *argv[])
{
    ...
    tag tags={'\0'};
    FILE *fptr = fopen(argv[2],"r");                                       //FIle opened in read mode
    char *label[] = {"TIT2","TPE1","TALB","TYER","TCON","COMM"};           //array of strings containing tags
    if(fptr == NULL)                                                      //FIle is opened or not
	return FAILURE;
    char temp[5] = {0};
    char version[3];
    int size;
    fread(temp,3,1,fptr);                                               //1st 3bytes of header is id3
    if(!strcmp(temp,"ID3"))
    {
	fread(version,2,1,fptr);                                       //next 2bytes for version
	if(version[0] == 0x03 && version[1] == 0x00)                   //comparing the version is 0x0300
	{
	    fseek(fptr,10,SEEK_SET);                                     //skip header
	    int i,j;
	    for(i = 0; i<6; i++)                                       //Loop for reading all 6 tags
	    {
		fread(temp,4,1,fptr);                                  //each tag is of 4bytes
		size = read_size(&fptr,4) - 1;                         //next bytes for size (-1 for reducing 1 flag byte)
		fseek(fptr,3,SEEK_CUR);                                                       //skip flags
		for(j = 0; j <6; j++)
		{
		    if(strcmp(temp,label[j])==0)                        //Check if read tag is in the 6 tags 
		    {
			break;
		    }
		...
		switch(j)                                               //read the bytes size times to corresponding string from structure             
		{
			...
			break;
		    default:
			fseek(fptr, size, SEEK_CUR);               //If tags are not matching skip size times and continue the iteration
		}
	    }
